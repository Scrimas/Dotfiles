...
Singleton {
    ...
    property list<var> apps: {
        ...
        // Separator
        const pinnedAppsLower = pinnedApps.map(id => id.toLowerCase());
        const hasUnpinnedApps = ToplevelManager.toplevels.values.some(toplevel => {
            return !ignoredRegexes.some(re => re.test(toplevel.appId)) &&
                   pinnedAppsLower.indexOf(toplevel.appId.toLowerCase()) === -1;
        });

        if (pinnedApps.length > 0 && hasUnpinnedApps) {
