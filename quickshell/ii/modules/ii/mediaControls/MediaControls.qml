...
Scope {
        ...
        sourceComponent: PanelWindow {
            ...
            margins {
                ...
                left: {
                    ...
                    let anchorX = GlobalStates.mediaControlsAnchorX;
                    if (anchorX === 0) {
                        return (panelWindow.screen.width - root.widgetWidth) / 2;
                    }
                    
                    let targetLeft = anchorX - (root.widgetWidth / 2);
                    
                    // Clamp to screen bounds with some margin
                    let margin = Appearance.sizes.hyprlandGapsOut;
                    return Math.max(margin, Math.min(targetLeft, panelWindow.screen.width - root.widgetWidth - margin));
                }
    ...
    IpcHandler {
        ...
        function toggle(): void {
            if (root.meaningfulPlayers.length === 0 && !GlobalStates.mediaControlsOpen) return;
            if (!GlobalStates.mediaControlsOpen) GlobalStates.mediaControlsAnchorX = 0;
        ...
        function open(): void {
            if (root.meaningfulPlayers.length === 0) return;
            GlobalStates.mediaControlsAnchorX = 0;
